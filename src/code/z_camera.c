            ...
            if (rwData->curKeyFrame->initField != ONEPOINT_CS_INIT_FIELD_NONE) {
                if (ONEPOINT_CS_INIT_FIELD_IS_TYPE_ACTORCAT(rwData->curKeyFrame->initField)) {
                    ...
                    if ((camera->player->stateFlags1 & PLAYER_STATE1_SWIMMING ) &&
                        ...
                        Player_SetCsAction(camera->play, camera->target, PLAYER_CSMODE_WAIT);
    ...
    if (camera->player->stateFlags1 & PLAYER_STATE1_SWIMMING  && (player->currentBoots != PLAYER_BOOTS_IRON)) {
        ...
        if (camera->target->category == ACTORCAT_PLAYER) {
            ...
            if (player->stateFlags1 & PLAYER_STATE1_CARRYING_ACTOR) {
                ...
                Player_SetCsActionWithHaltedActors(camera->play, camera->target, PLAYER_CSMODE_WAIT);
            } else if (ABS(framesDiff) > 3000) {
                Player_SetCsActionWithHaltedActors(camera->play, camera->target, PLAYER_CSMODE_FIGHT_STANCE);
            } else {
                Player_SetCsActionWithHaltedActors(camera->play, camera->target, PLAYER_CSMODE_LOOK_AROUND_AND_AT_SELF_QUICKLY);
            }
        } else {
            Player_SetCsActionWithHaltedActors(camera->play, camera->target, PLAYER_CSMODE_IDLE);
        }
    }
    ...
    switch (camera->animState) {
        ...
        case 1:
            if (stateTimers[camera->animState] < rwData->animTimer) {
                Player_SetCsActionWithHaltedActors(camera->play, &camera->player->actor, PLAYER_CSMODE_WAIT);
                focusPosRot = Actor_GetWorld(camFocus);
    ...
    if (camera->timer == 0) {
        ...
        if ((camera->parentCamId == CAM_ID_MAIN) && (camera->csId != 0)) {
            ...
            if (player->csAction != PLAYER_CSACTION_NONE) {
                Player_SetCsActionWithHaltedActors(camera->play, &player->actor, PLAYER_CSMODE_END);
